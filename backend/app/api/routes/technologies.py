...
def create_technology(payload: TechnologyCreate, db: DBSession, admin_user: AdminUser):
    return technologies_service.create_technology(db, payload)


@router.patch("/{technology_id}", response_model=TechnologyRead)
def update_technology(technology_id: int, payload: TechnologyUpdate, db: DBSession, admin_user: AdminUser):
