...
def update_contact_message(message_id: int, payload: ContactMessageUpdate, db: DBSession):
    return contact_messages_service.update_contact_message(db, message_id, payload)


@router.delete("/{message_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_contact_message(message_id: int, db: DBSession):
